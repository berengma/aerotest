...
function aerotest.spawnstep(dtime)
	...
	for _,plyr in ipairs(minetest.get_connected_players()) do
		...
		if coin < aerotest.spawnchance then
			if plyr and plyr:get_pos().y > aerotest.eagleminheight 
				and plyr:get_pos().y < 500 then
				local pos = plyr:get_pos()
				local yaw = plyr:get_look_horizontal()
				local animal = water_life.count_objects(pos,nil,"aerotest:eagle")
				if not animal["areotest:eagle"] or 
					animal["areotest:eagle"] < aerotest.maxeagle then
					pos = mobkit.pos_translate2d(pos,yaw+rad(random(-55,55)),
						random(10,aerotest.abr))
					local spawnpos = {x=pos.x, y=pos.y + random(
						aerotest.abr/2,aerotest.abr), z=pos.z}
					...
					if obj then
						...
						velo = vector.subtract(
							mobkit.pos_translate2d(velo,yaw,2),velo)
